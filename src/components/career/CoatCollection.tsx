...
export function CoatCollection({ completedSet, careerStats, visible }: CoatCollectionProps) {
      ...
      {selected && (() => {
        ...
        return (
          <div className="coat-collection-card-wrap">
            <div className="coat-collection-card" onClick={() => setSelected(null)}>
              <img
        ...
        {regions.map(({ region, names, completed }) => (
          <section key={region} className="coat-collection-region">
            <h3 className="coat-collection-region-header">
              {region} <span className="coat-collection-count">{completed}/{names.length}</span>
            </h3>
              ...
              {names.map((name) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => done && setSelected(selected === name ? null : name)}
